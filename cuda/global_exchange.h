...
void fmoe_cuda_global_scatter_impl(
    ...
    for (size_t i = 0; i < n_expert; ++i) {
        ...
        for (size_t j = 0; j < world_size; ++j) {
            ...
            if (local_expert_count[idx]) {
                NCCL_SAFE_CALL(ncclSend(
                        ...
                        local_expert_count[idx] * in_feat / 4 * sizeof(scalar_t),
                        ncclChar, 
                        j,
                        smgr->ncclcomm,
                        smgr->stream(0)));
            ...
            if (global_expert_count[idx]) {
                NCCL_SAFE_CALL(ncclRecv(
                        ...
                        global_expert_count[idx] * in_feat / 4 * sizeof(scalar_t),
                        ncclChar,
                        j,
                        smgr->ncclcomm,
                        smgr->stream(0)));
        ...
        for (size_t j = 0; j < world_size; ++j) {
            ...
            if (local_expert_count[idx]) {
                NCCL_SAFE_CALL(ncclSend(
                        local_input_buf + expert_ptr[idx] * in_feat + local_expert_count[idx] * in_feat / 4,
                        local_expert_count[idx] * in_feat / 4 * sizeof(scalar_t),
                        ncclChar, 
                        j,
                        smgr->ncclcomm,
            ...
            if (global_expert_count[idx]) {
                NCCL_SAFE_CALL(ncclRecv(
                        input_buf + recv_ptr2 * in_feat + global_expert_count[idx] * in_feat / 4,
                        global_expert_count[idx] * in_feat / 4 * sizeof(scalar_t),
                        ncclChar,
                        j,
                        smgr->ncclcomm,
                        smgr->stream(0)));
                recv_ptr2 += global_expert_count[idx];
            }
        }
        NCCL_SAFE_CALL(ncclGroupEnd());
        NCCL_SAFE_CALL(ncclGroupStart());
        for (size_t j = 0; j < world_size; ++j) {
            ...
            if (local_expert_count[idx]) {
                NCCL_SAFE_CALL(ncclSend(
                        local_input_buf + expert_ptr[idx] * in_feat + local_expert_count[idx] * in_feat / 2,
                        local_expert_count[idx] * in_feat / 4 * sizeof(scalar_t),
                        ncclChar, 
                        j,
                        smgr->ncclcomm,
            ...
            if (global_expert_count[idx]) {
                NCCL_SAFE_CALL(ncclRecv(
                        input_buf + recv_ptr3 * in_feat + global_expert_count[idx] * in_feat / 2,
                        global_expert_count[idx] * in_feat / 4 * sizeof(scalar_t),
                        ncclChar,
                        j,
                        smgr->ncclcomm,
                        smgr->stream(0)));
                recv_ptr3 += global_expert_count[idx];
            }
        }
        NCCL_SAFE_CALL(ncclGroupEnd());
        NCCL_SAFE_CALL(ncclGroupStart());
        for (size_t j = 0; j < world_size; ++j) {
            ...
            if (local_expert_count[idx]) {
                NCCL_SAFE_CALL(ncclSend(
                        local_input_buf + expert_ptr[idx] * in_feat + local_expert_count[idx] * in_feat * 3 / 4,
                        local_expert_count[idx] * in_feat / 4 * sizeof(scalar_t),
                        ncclChar, 
                        j,
                        smgr->ncclcomm,
            ...
            if (global_expert_count[idx]) {
                NCCL_SAFE_CALL(ncclRecv(
                        input_buf + recv_ptr4 * in_feat + global_expert_count[idx] * in_feat * 3 / 4,
                        global_expert_count[idx] * in_feat / 4 * sizeof(scalar_t),
                        ncclChar,
                        j,
                        smgr->ncclcomm,
                        smgr->stream(0)));
                recv_ptr4 += global_expert_count[idx];
            }
        }
        NCCL_SAFE_CALL(ncclGroupEnd());
    }
